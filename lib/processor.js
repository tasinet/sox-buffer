...
module.exports = function(proto) {
	...
	proto._getArguments = function() {
		...
		var outputArguments = this._outputs.reduce(function(args, output) {
			...
			if (!output.target) {
				outputArg = [];
			} else if (typeof output.target === 'string' && output.target !== 'buffer') {
	...
	proto.run = function() {
		...
		function emitEnd(err, stdout, stderr, buffer) {
			if (!ended) {
				ended = true;
				if (err) {
					self.emit('error', err, stdout, stderr);
				} else {
					self.emit('end', stdout, stderr, buffer);
				}
			}
		}
			...
			self._spawnSox(
				...
				function endCB(err) {
					...
					if (err) {
						...
						var buffer = outputStream && outputStream.buffer && Buffer.concat(outputStream.buffer)
						emitEnd(null, stdout, stderr, buffer);
					}
